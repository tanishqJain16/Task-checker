...
function TaskCard(props) {
    ...
    const handleDelete = async (e) => {
        e.preventDefault();
        const response = await fetch(`http://localhost:5000/user/deletetask/${props.id}`, {         // eslint-disable-line
            method: "GET",
            headers: {
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        setDisable(true);
        const response = await fetch(`http://localhost:5000/user/updatetask/${props.id}`, {         //eslint-disable-line
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${localStorage.getItem('token')}`
